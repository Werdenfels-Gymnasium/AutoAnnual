...
public class Util {
	...
	public static void appendTab(Element x, ScribusXML xml) {
		x.appendChild(xml.getRootDocument().createElement("tab"));
	}
	
	public static void appendPara(Element x, ScribusXML xml, String parent) {
		Element para = xml.getRootDocument().createElement("para");
		para.setAttribute("PARENT", parent);
		x.appendChild(para);
	}
	
	public static void appendTrail(Element x, ScribusXML xml, String parent) {
		Element para = xml.getRootDocument().createElement("trail");
		para.setAttribute("PARENT", parent);
		x.appendChild(para);
	}
	...
	public static void createPages(ScribusXML xml, int page) {
		Element pageobj = xml.getRootDocument().createElement("PAGE");
		...
		Element pageobj2 = xml.getRootDocument().createElement("PAGE");
	...
	public static String formatClassName(String input) {
		final int num = Integer.parseInt(input.replaceAll("[a-zA-Z]+", ""));
		return classformat.format(num) + input.replaceAll("[0-9]+", "");
	}
}
